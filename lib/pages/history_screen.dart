...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    if (authProvider.token == null) {
      _setError('Please login to view reading history');
      return;
    }
  ...
  Future<void> _loadMoreHistory() async {
    ...
    try {
      ...
      if (response is Map<String, dynamic>) {
        ...
        final List<History> newHistories = historyData.map<History>((item) {
          return History(
            mangaId: item['manga_id']?.toString() ?? '',
            chapterId: item['chapter_id']?.toString() ?? '',
            lastPage: int.tryParse(item['last_page']?.toString() ?? '0') ?? 0,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to load more history: ${e.toString()}'),
  ...
  void _navigateToComicDetail(History history) {
    ...
    final comic = Comic(
      ...
      coverUrl: history.mangaCoverUrl ?? '',
      author: '', // Will be loaded in detail screen
      artist: '', // Will be loaded in detail screen
      status: '', // Will be loaded in detail screen
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Start reading some comics to see your reading history here!',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 16,
                color: Colors.grey[600],
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Text(
                '${_pagination?.totalItems ?? _histories.length} Manga Read',
                style: TextStyle(
                  fontSize: 16,
  ...
  Widget _buildHistoryCard(History history) {
    return Container(
      ...
      child: InkWell(
        onTap: () => _navigateToComicDetail(history),
        borderRadius: BorderRadius.circular(12),
        child: Padding(
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: Container(
                  ...
                  child: history.mangaCoverUrl != null && history.mangaCoverUrl!.isNotEmpty
                      ? Image.network(
              ...
              // Comic info - FIXED: Added Expanded to prevent overflow
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
